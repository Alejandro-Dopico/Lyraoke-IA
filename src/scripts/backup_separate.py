...
def ensure_proper_shape(audio: torch.Tensor) -> torch.Tensor:
    """
    Garantiza que el tensor de audio tenga la forma correcta para torchaudio.save
    """
...
def separate_audio(input_filename: str = None, output_dir: Path = OUTPUT_DIR, model_path: Path = MODEL_PATH) -> bool:
    """
    Versión corregida que maneja correctamente la carga del modelo
    """
    ...
    try:
        ...
        # 4. Cargar modelo - MANEJO MEJORADO DE LA TUPLA
        logger.info(f"Cargando modelo: {model_path}")
        try:
            model = load_model(str(model_path), device, weights_only=True)
        ...
        torchaudio.save(str(output_dir / "instrumental.wav"), instrumental, target_sr)
        torchaudio.save(str(output_dir / "vocals.wav"), vocals, target_sr)
